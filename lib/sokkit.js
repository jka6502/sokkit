(function() {
...

	{
		...
		enable: function(module) {
			...
			if (!details) { throw new Error('Unknown plugin: ' + module); }
		...
		depend: function(retrieve) {
			...
			do{
				...
				for(var module in modules) {
					if (!modules.hasOwnProperty(module)) { continue; }
					var requires = retrieve(module, modules[module]);
					if (!requires) { continue; }

					for(var index = 0, length = required.length;
							index < length; index++) {
						var depend = requires[index];
						if (!modules[depend]) {
							if (fail(module, modules[index], depend) !== false) {
								errors.push({
									module: module,
